...
my_spark = SparkSession \
    ...
    .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/db_meteo.result_tempSQL")\
    .getOrCreate()
df = my_spark.read.format("mongo").option("uri","mongodb://127.0.0.1/db_meteo.meteo_provincie").load()
#df.show()
df.createOrReplaceTempView("meteo")
sqlDF = my_spark.sql("SELECT citta, avg(temp) AS TEMP_MEDIA FROM meteo GROUP BY citta")
